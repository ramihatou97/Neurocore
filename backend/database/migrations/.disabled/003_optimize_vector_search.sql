...
-- Note: authors is a TEXT[] array, skip GIN index (would need immutable function)
-- Search on authors can use regular array operators: authors @> ARRAY['Smith']
...
-- Note: chapters use JSONB sections instead of a summary column

-- ==================== Composite Indexes for Filtering ====================

-- PDF search with filters
CREATE INDEX IF NOT EXISTS idx_pdfs_status_created ON pdfs(indexing_status, created_at DESC);
CREATE INDEX IF NOT EXISTS idx_pdfs_year_created ON pdfs(publication_year DESC, created_at DESC);
